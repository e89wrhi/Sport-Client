  ...
  CredentialsProvider({
    ...
    async authorize(credentials) {
      ...
      if (useMockAuth) {
        ...
        // Return Sara Getachew's admin data (adm-002) to align with mock admins
        return {
          id: 'adm-002',
          userId: 'usr-002',
          name: 'Sara Getachew',
  ...
  callbacks: {
    ...
    async jwt({ token, account, profile }: any) {
      ...
      if (account && profile) {
        ...
        // Check if user has required role for admin portal
        ...
        try {
          ...
          const response = await fetch(tokenEndpoint, {
            ...
            body: new URLSearchParams({
              client_id:
                (useMockAuth ? 'admin-portal-mock' : AUTH_DUENDE_ID) || '',
              client_secret:
